...
If the proportion of people with a lower status in a neighborhood increases with one percentage point, the median value of owner-occupied homes in that neighborhood decreases with $\$`r round(coef(lm_ses)[2])`$.
...
The function `summary` gives the output of the linear regression model in a structured way, containing nearly all relevant information if the goal is just to interpret the model.
...
```{r}
y_pred_new <- predict(lm_ses, pred_dat)
```
...
```{r}
dat_95 <- tibble(lstat = pred_dat$lstat,
                 medv  = y_pred_95[, 'fit'],
                 lower = y_pred_95[, 'lwr'],
                 upper = y_pred_95[, 'upr'])
```
...
```{r}
model_2 <- update(model_1, ~ lstat + age + tax)
model_2_mse_train <- mse(boston_train$medv, predict(model_2))
model_2_mse_valid <- mse(boston_valid$medv, predict(model_2, boston_valid))
```
...
```{r}
model_2_mse_test <- mse(boston_test$medv, predict(model_2, boston_test))
```

This number tells me that on average, the squared error between the observed and predicted median houseprices within a region equals `r round(model_2_mse_test, 2)`.
...
k_fold(medv ~ lstat + age + tax, Boston, 9)
k_fold(medv ~ lstat + I(lstat^2) + age + tax, Boston, 9)
```
...
There are `r nrow(baseball)` players left.
...
baseball_test  <- baseball %>% filter(splits == "test")
```

### Exercise 3

```{r}
lm_mse <- function(formula, training_set, test_set) {
  train <- lm(formula, training_set)
  mean((model.frame(formula, test_set)[,1] - predict(train, test_set))^2)
}
```
...
```{r}
source("data/generate_formulas.R")

preds <- Hitters %>% dplyr::select(-Salary) %>% colnames()
```

### Exercise 6

```{r}
formulas <- generate_formulas(3, preds, "Salary")
```
...
formulas[which.min(best_subset_mse_3)]
```
...
best_models[which.min(lowest_mses)]
```
So in this case, the most complex model is the best model.
...
```{r}
lasso <- glmnet(x_train, y = baseball_train$Salary, 
                family = "gaussian", alpha = 1, lambda = 15, )
```
...
as.matrix(coefs)[abs(as.matrix(coefs)) > 0,]
```
...
mean((baseball_valid$Salary - lasso_pred)^2)
```
...
```{r}
pn3_mod <- lm(medv ~ poly(lstat, 3), Boston)
pred_plot(pn3_mod)
```
...
```{r}
poly(Boston$lstat, 3, raw = TRUE) %>% head()
```

### Exercise 5

By default, `poly(raw = FALSE)` computes an orthogonal polynomial. This does not change the fitted values, but it allows you to check whether a certain order in the polynomial significantly improves the regression over the lower orders.

```{r}
summary(lm(medv ~ poly(lstat, 7), Boston))
```
...
A low `lstat` neighborhood has a higher `medv` value. That is, a neighborhood with `lstat` under the median of `lstat` over all neighborhoods has a median houseprice that is 11.7 thousand dollar more expensive than in a neighborhood where `lstat` is higher than the median.
...
Sections generated by `cut()` are equally spaced in terms of the predictor, but they do not have equal amounts of data.

### Exercise 8

```{r}
breaks <- c(-Inf, quantile(Boston$lstat, seq(0.2,0.8, by = 0.2)), Inf)

pwq_mod <- lm(medv ~ cut(lstat, breaks = breaks), Boston)
...
piecewise_cubic_basis <- function(vec, knots = 1) {
  ## if the number of knots is zero, there are no breakpoints,
  ## and a simple cubic regression model is fitted
  if (knots == 0) return(poly(vec, degree = 3, raw = TRUE))
  
  ## create a cut vector k + 1 categories
  cut_vec <- cut(vec, breaks = knots + 1)
  
  ## empty matrix with the number of rows equal to the input vector
  out <- matrix(nrow = length(vec), ncol = 0)
  
  ## then, for all levels in the cut_vec (that is, the number of categories)
  ## take over the input variable, set all values outside the breakpoint
  ## to zero, and create the output matrix with values for individuals within
  ## that category, and a zero on all these variables outside that category
...
```{r}
pc1_mod <- lm(medv ~ piecewise_cubic_basis(lstat, 1), Boston)
pc2_mod <- lm(medv ~ piecewise_cubic_basis(lstat, 2), Boston)
pc3_mod <- lm(medv ~ piecewise_cubic_basis(lstat, 3), Boston)
...
```{r}
boston_tpb <- Boston %>% dplyr::select(medv, lstat)
```
...
predict(tpb_mod)[1:10]
predict(bs1_mod)[1:10]
```

They produce exactly similar results.
...
```{r}
cowplot::plot_grid(pred_plot(lin_mod) + ggtitle("Linear model"),
                   pred_plot(pn3_mod) + ggtitle("Third-degree polynomial"),
                   pred_plot(pw5_mod) + ggtitle("Five-piece regression"),
                   pred_plot(pc3_mod) + ggtitle("Three-piece cubic"),
                   pred_plot(bs1_mod) + ggtitle("Median knot basic spline"),
                   pred_plot(ns3_mod) + ggtitle("Three df natural spline"),
                   ncol = 2)
```
...
```{r, warning = FALSE}
pw5_mod <- lm(medv ~ cut(lstat, breaks = c(-Inf, 7, 15, 22, Inf)), Boston)

list(lin_mod, pn3_mod, pw5_mod, pc3_mod, bs1_mod, ns3_mod) %>%
  map_dbl(function(x) {
    
    dat <- Boston
    dat$split <- sample(rep_len(1:12, length.out = nrow(Boston)))
    
    map_dbl(1:12, function(i) {
      train <- dat[dat$split != i,]
      test  <- Boston[dat$split == i,]
      mod <- lm(x$call$formula, train)
      mean((test$medv - predict(mod, test))^2)
    }) %>% mean()
  })
```
